...
describe("then", () => {
    ...
    it("with chaining", () => {
        return promise({ value: 3 }).then(v => v * 4).then(v => expect(v).toEqual(12));
    });
...
describe("catch", () => {
    ...
    it("with chaining", () => {
        return promise({ value: 3 }).then(v => { throw v * 4; }).catch(v => expect(v).toEqual(12));
    });
...
describe("finally", () => {
    ...
    it("with chaining", () => {
        const checkFunc = v => v => expect(v).toBeUndefined();
        const successPromise = promise().then(v => v).finally(checkFunc);
        const failPromise = promise({ fail: true }).then(v => v).finally(checkFunc);
        return Promise.allSettled([successPromise, failPromise]);
    });
...
describe("static methods", () => {
    ...
    it("allSettled", () => {
        return MyPromise.allSettled([promise(), promise({ fail: true })]).then(v => expect(v).toEqual([
    ...
    describe("race", () => {
        ...
        it("with fail", () => {
            return MyPromise.race([promise({ fail: true, value: 1 }), promise({ fail: true, value: 2 })]).catch(v => expect(v).toEqual(1));
        });
